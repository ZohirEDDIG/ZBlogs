    ...
    return (
        <div className='grid grid-cols-1 min-[700px]:grid-cols-2 items-center gap-10'>
            ...
            <div className='flex flex-col gap-y-4'>
                ...
                <Link to={`/blog/${blog.blogId}`}  className='text-2xl line-clamp-1'>{blog.title}</Link>

                <Link to={`/blog/${blog.blogId}`} className='text-sm line-clamp-2'>{blog.description}</Link>
                ...
                <div className='flex flex-wrap items-center gap-2'>

                    {
                        ...
                        blog.topics.map((topic, index) => {
                            ...
                            return <span key={index} className='text-sm text-gray-500'>#{topic}</span>;

                        })

                    }

                    <button className='w-fit text-gray-500 ml-4 flex gap-x-2 transition-all duration-300 ease-in-out hover:text-gray-800' type='button'>
